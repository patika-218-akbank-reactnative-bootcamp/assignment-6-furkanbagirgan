...
const Home = ({navigation}) => {
  ...
  const withCamera = async () => {
    if (user.image === '') {
      ...
      Alert.alert(
        'Warning',
        'A picture has already been shared. To share a new picture, you must delete the previous one.',
      );
    }
  };
    ...
    <SafeAreaView style={styles[theme].container}>
      ...
      <View style={styles[theme].imageContainer}>
        <View style={styles[theme].imageWrapper}>
          {user.image !== '' ? (
            <Image source={{uri: user.image}} style={styles[theme].image} />
          ) : (
            <Icon
              name="x"
              color={theme === 'light' ? '#B9C0C8' : '#F2F2F2'}
              size={32}
            />
          )}
        </View>
        <Text style={styles[theme].imageTitle}>
          {user.image !== ''
            ? 'The picture has been shared.'
            : 'There are no shared images.'}
        </Text>
        {user.image !== '' && (
          <View style={styles[theme].iconWrapper}>
            <Icon name="trash" color="white" size={20} onPress={deletePhoto} />
          </View>
        )}
      </View>
    </SafeAreaView>
